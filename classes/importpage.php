...
class ImportPage extends RunnerPage
{
	...
	protected function prepareAndSentPreviewData()
	{
		...
		if( $returnJSON != false )
			...
			echo "Il file che si sta tentando di importare non può essere analizzato";
...
//		$importFields = $this->pSet->getImportFields();
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= "Importa in"." ".$this->strOriginalTableName.$lineBreak.
				str_format_datetime( db2time( now() ) ) .$lineBreak.$lineBreak;
		}
		
		$reportText .= mysprintf("%s di %s records elaborati con esito positivo.", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s records inseriti.", array($boldBegin.$addedRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s records aggiornati.", array($boldBegin.$updatedRecords.$boldEnd)) .$lineBreak;
			
		if( $notImportedRecords )
			$reportText.= mysprintf("%s records processati in errore", array($boldBegin.$notImportedRecords.$boldEnd));
