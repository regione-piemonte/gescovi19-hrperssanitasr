...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if (!$file["name"]) {
            $file["error"] = "Il file non è stato fornito";
            return false;
        }
        if (!preg_match($this->options['accept_file_types'], $file["name"])) {
            $file["error"] = "Il tipo di file non è accettabile";
            return false;
        }
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("La dimensione del file supera il limite di %s kbytes", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("La dimensione del file non dev'essere inferiore a %s kbytes", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("La dimensione totale dei file supera il limite di %s kbytes", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("Puoi fare l'upload di max %s file", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "Puoi fare l'upload di un solo file";
            return false;
        }
    ...
    public function handle_file_upload($uploadedFile, $index) {
		...
		if($this->pSet->isMakeDirectoryNeeded($this->field))
		{
			...
			{
				$file["error"] = "La cartella di upload non esiste";
				return $file;
			}
		}
