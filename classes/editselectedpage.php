...
class EditSelectedPage extends EditPage
{
	...
	protected function prepareButtons()
	{
		...
		{
			$label = str_replace( "%n%", $this->nSelected, "Aggiornare %n% record" );
	...
	{
		$message = str_replace( array("%succeed%", "%total%"), array( "<strong>".$this->nUpdated."</strong>", "<strong>".$this->nSelected."</strong>" ), "%succeed% su %total% record aggiornati correttamente.");
		$this->setMessage( $message );
		
		if( $this->nUpdated != $this->nSelected ) {
			$message = str_replace( "%failed%", "<strong>".($this->nSelected - $this->nUpdated)."</strong>" , "%failed% record fallito.");
			$this->setMessage( $message );
		}
	}
