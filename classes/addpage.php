...
class AddPage extends RunnerPage
{
	...
	public static function handleBrokenRequest()
	{
		...
		if( postvalue("inline") ) 
		{
			...
			$returnJSON['message'] = "Si é verificato un errore";
	...
	protected function recheckUserPermissions()
	{
		...
		{
			$this->setMessage( "La tua sessione è scaduta." 
				. "<a href='#' id='loginButtonContinue" . $this->id . "'>" . "Entra" . "</a>" 
				. "per salvare i dati" );
		}
		else
		{
	...
	protected function setSuccessfulUpdateMessage()
	{
		...
		if( $this->mode == ADD_INLINE ) 
			$infoMessage = ""."Il Record è stato aggiunto"."";
		else
			$infoMessage = "<strong><<< "."Il Record è stato aggiunto"." >>></strong>";
		...
		else 
		{
			...
			if( $this->editAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "edit", $keylink )."'>"."Modifica"."</a>&nbsp;";
				
			if( $this->viewAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "view", $keylink )."'>"."Dettagli"."</a>&nbsp;";
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != ADD_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."Il record non è stato aggiunto"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
		{
			$this->message = "Il record non è stato aggiunto".". ".$message;
		}
