...
class EditPage extends RunnerPage
{
	...
	public static function processEditPageSecurity( $table )
	{
		...
		if( $pageMode != EDIT_SIMPLE )
		{
			...
			if( !isLogged() || isLoggedAsGuest() )
				$messageLink = " <a href='#' id='loginButtonContinue'>". "Entra" . "</a>";
	...
	public static function handleBrokenRequest()
	{
		...
		else
		{
			if( postvalue('fly') )
			{
				...
				$_SESSION["message_edit"] = "<< "."Si é verificato un errore"." >>";
		}
	}
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."Record aggiornato". " &gt;&gt;&gt;</strong>" );
	}
	...
	protected function SecurityRedirect()
	{
		if($this->mode == EDIT_INLINE)
		{
			echo printJSON(array("success" => false, "message" => "Il record non è editabile"));
			exit();
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."Il record non è stato modificato"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "Il record non è stato modificato".". ".$message;
