...
//				$mParams["pageType"] = $mParams["masterPageType"];
	...
	function assignBodyEnd($params = "")
	{
		...
		echo "<script language=\"JavaScript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?34075")."\"></script>\r\n";
...
*/
	...
	function checkCaptcha()
	{
		...
		if ( $captchaSettings["type"] == FLASH_CAPTCHA && @strtolower($this->captchaValue) != strtolower(@$_SESSION["captcha_" . $this->getCaptchaId()]) )
		{
			...
			$this->message = "Codice di sicurezza non valido.";
		}
	...
	function getCaptchaHtml($_captchaFieldName)
	{
		...
		$typeCodeMessage = "Digita il codice che vedi di seguito";
		$path = GetCaptchaPath();
	...
	function noRecordsMessage()
	{
		...
		if( $this->pSetSearch->noRecordsOnFirstPage() && !$isSearchRun )
			return "Niente da vedere. Eseguire alcune ricerche.";

		if( !$this->rowsFound && !$isSearchRun )
			return "Dati non presenti";

		if( $isSearchRun && !$this->rowsFound )
			return "Non sono stati trovati Record";
	}
	...
	function buildPagination()
	{
		...
		else
		{
			...
			if($this->maxPages > 1)
			{
				...
				if($counterstart != 1)
				{
					$pagination.= $this->getPaginationLink(1,"Primo") . $advSeparator;
					$pagination.= $this->getPaginationLink($counterstart - 1,"Precedente").$separator;
				}
				$pageLinks = "";
				...
				if($counterend != $this->maxPages)
				{
					$pagination.= $separator . $this->getPaginationLink($counterend + 1,"Prossimo") . $advSeparator;
					$pagination.= $this->getPaginationLink($this->maxPages,"Ultimo");
				}
				if( $this->isBootstrap() )
	...
	function prepareRecordsIndicator($firstDisplayed, $lastDisplayed, $totalDisplayed)
	{
		...
		{
			$template = "Visualizzati %first% - %last% di %total%";
	...
	*/
	...
	function getProceedLink()
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "Procedi a" . ' '. GetTableCaption( GoodFieldName( $this->tName ) )
			. '</a>'
			. "&nbsp;&nbsp;</span>";
	}
...
//		$this->AddJSFile('libs/js/migrationTool.js');
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."Aggiungi nuovo";
		if( $page == "edit" )
			return GetTableCaption($table).", "."Modifica"." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table)." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "Esporta";
		if( $page == "import" )
			return GetTableCaption($table).", "."Importa";
		if( $page == "search" )
			return GetTableCaption($table)." - "."Ricerca avanzata";
		if( $page == "print" )
