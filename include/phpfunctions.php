...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="hostname.covidhr_t_fabbisogno" && $field=="minimo_posti_rich")
	{
		global $conn;

$sql="select c.nome nome_ente ,
d.nome area,
sum(e.posti_target) posti_target,
sum(e.posti_disponibili) posti_disponibili
from area a, struttura b, ente c, d_area d, disponibilita e
where a.id_struttura=b.id_struttura
and b.id_ente=c.id_ente
and d.id_d_area=a.id_d_area
and e.id_area=a.id_area
and e.flag_valido='SI'
and c.id_ente=" . $_SESSION['id_ente'] ."
and d.id_d_area='" . $data["id_d_area"] . "' 
group by c.nome ,
d.nome";
