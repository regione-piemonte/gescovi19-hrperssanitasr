...
class oLocking
{
	...
	function __construct()
	{
		...
		$this->ConfirmAdmin = "L'amministratore %s ha chiuso la tua sessione di editing";
		$this->ConfirmUser = "La tua sessione di editing è scaduta";
		$this->LockAdmin = "Il record è stato modificato da %s per %s minuti";
		$this->LockUser = "Record modificato da un altro utente";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."Sblocca record".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."Modifica record".'</a>';
			}
			return $str;
		}
