...
function checkTableName($shortTName, $type=false)
{
	...
	if ("covidhr_t_fabbisogno" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("area" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("r_utente_area" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("r_utente_ente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("r_utente_profilo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("d_area" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("r_utente_struttura" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("utenti" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("covidhr_t_fabbisogno_det" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("covidhr_d_fabbisogno_personale_tipo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("covidhr_d_fabbisogno_stato" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.covidhr_t_fabbisogno");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.covidhr_t_fabbisogno";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.area");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.ente");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.r_utente_area");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.r_utente_area";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.r_utente_ente");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.r_utente_ente";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.r_utente_profilo");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.r_utente_profilo";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.d_area");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.r_utente_struttura");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.r_utente_struttura";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.utenti");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.covidhr_t_fabbisogno_det");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.covidhr_t_fabbisogno_det";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.covidhr_d_fabbisogno_personale_tipo");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.covidhr_d_fabbisogno_personale_tipo";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hostname.covidhr_d_fabbisogno_stato");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hostname.covidhr_d_fabbisogno_stato";
	}
	return $arr;
}
...
function GetUserPermissionsStatic( $table )
{
	...
	if( $table=="hostname.covidhr_t_fabbisogno" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.area" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.ente" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.r_utente_area" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.r_utente_ente" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.r_utente_profilo" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.d_area" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.r_utente_struttura" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.utenti" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
...
//	default permissions
	...
	if( $table=="hostname.covidhr_d_fabbisogno_personale_tipo" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="hostname.covidhr_d_fabbisogno_stato" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
...
{
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?34075")."\"></script>";
	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?34075")."\"></script>";
...
function verifyRecaptchaResponse( $response ) {
	...
	$errors = array();
	$errors["missing-input-response"] = "Codice di sicurezza non valido.";
	$errors["invalid-input-response"] = "Codice di sicurezza non valido.";
